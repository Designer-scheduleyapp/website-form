...
location.href = base + 'Arabic/index.html';
...
let total = siteTypeCost + (hostingSelected ? PRICE_HOSTING : 0) + emailCost;
  ...
  function filterTemplates() {
   ...
   // const selectedFeatures = [...document.querySelectorAll('.feature-option.selected')].map(el => el.dataset.value);
...
//console.log('Selected features:', selectedFeatures);
    ...
    if (siteType === 'custom') {
      ...
      message = 'Template selection is not available for custom websites. Please proceed to the next step.';
      ...
      if (selectedFeatures.length === 0) {
        ...
        const templateMatches = templates.map(t => {
         ...
         // const count = selectedFeatures.filter(f => features.includes(f)).length;
      /*   const count = selectedFeatures.filter(f =>
          features.some(t => t.toLowerCase() === f.toLowerCase())
        ).length; */

      //  console.log('Template:', t.dataset.templateId, 'Features:', features); // ✅ Add this
        ...
        if (fullTemplates.length > 0) {
          ...
          message = 'Showing templates that match all selected features.';
        } else {
          ...
          message = 'No template has all selected features. Showing partial matches.';
        }
  ...
  document.getElementById('clearFilters').addEventListener('click', () => {
    ...
    document.getElementById('filterMessage').innerText = 'Showing all templates (filters cleared).';
    document.getElementById('clearFilters').style.display = 'none';
  });
    ...
    .then(data => {
      if (data.success) {
        ...
        html2canvas(document.getElementById('previewInvoice')).then(canvas => {
          ...
          alert('Invoice sent and PDF downloaded!');
        });
      } else {
